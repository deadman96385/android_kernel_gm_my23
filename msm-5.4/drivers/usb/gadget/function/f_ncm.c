 ...
 */
#define MIN_NTB_RANGE		2048
#define MAX_NTB_RANGE		65536
...
static ssize_t ncm_opts_max_ntb_in_show(struct config_item *item, char *page) {
	return sprintf(page, "%d", le32_to_cpu(ntb_parameters.dwNtbInMaxSize));
}

static ssize_t ncm_opts_max_ntb_in_store(struct config_item *item,
	...
	strncpy(ntb_in, page, min((size_t)ARRAY_SIZE(ntb_in), len + 1));
	kstrtoint ( ntb_in, 0, &val);
	new = clamp(val, MIN_NTB_RANGE, MAX_NTB_RANGE);
        if ( new == val )
		ntb_parameters.dwNtbInMaxSize = cpu_to_le32(new);
	mutex_unlock(&opts->lock);
	return len;
}

CONFIGFS_ATTR (ncm_opts_, max_ntb_in);

static ssize_t ncm_opts_max_ntb_out_show(struct config_item *item, char *page) {
	return sprintf(page, "%d", le32_to_cpu(ntb_parameters.dwNtbOutMaxSize));
}

static ssize_t ncm_opts_max_ntb_out_store(struct config_item *item,
	const char *page, size_t len) {
	...
	mutex_lock(&opts->lock);
	strncpy(ntb_out, page, min((size_t)ARRAY_SIZE(ntb_out), len + 1));
	kstrtoint ( ntb_out, 0, &val);
	new = clamp(val, MIN_NTB_RANGE, MAX_NTB_RANGE);
	if ( new == val )
		ntb_parameters.dwNtbOutMaxSize = cpu_to_le32(new);

	mutex_unlock(&opts->lock);
	return len;
}

CONFIGFS_ATTR (ncm_opts_, max_ntb_out);
...
static struct configfs_attribute *ncm_attrs[] = {
	...
	&ncm_opts_attr_max_ntb_in,
	&ncm_opts_attr_max_ntb_out,
	NULL,
};
